...
if($_POST){
    ...
    $fields = [
        ...
        'gh', 'sst1', 'sst2', 'sst3', 'sst4', 'sst5', 'sst6', 'sst7', 'sst8', 'ti', 'ti1', 'ti2', 
        'ti3', 'mant', 'mant1', 'mant2', 'mant3', 'mant4', 'bpm', 'desp', 'dorp', 'dorp1', 
        'fa_mesas', 'fa_dom', 'fa_rappi', 'tesor', 'rappi', 'domi', 'tp', 'domiexpress', 
        'hdomi', 'pd', 'supervisores', 'act_sup', 'hielo', 'hielo1', 'hielo2', 'hielo3', 
        'hielo4', 'hielo5', 'devo', 'tesor1', 'bpm1', 'bpm2', 'bpm3', 'bpm4', 'bpm5', 'bpm6',
        'bpm7', 'bpm8', 'equipo_bpm', 'visita_ss', 'visita_dagma', 'visita_west', 'novedad_grameras',
        ...
        'facturas_mesas','facturas_domic','facturas_rappi','bonos_coomeva','reservas_15',
        'mer4','easypedido','tesor2','planta_elect','mant5','mant6','mant7','mant8',
    ...
    if ($bonos_coomeva === 'No') {
        ...
        if (isset($_POST['tesor1']) && trim($_POST['tesor1']) === $DEFAULT_FACTURAS) {
            $_POST['tesor1'] = '';
        }
    }
    ...
    function hasEmptyFields($data, $fields) {
        foreach ($fields as $field) {
            if (isset($data[$field])){
                ...
                if ($field === 'gh') continue;
                
                // CAMPOS MEJORAMIENTO
                if ($field === 'bpm1' && isset($data['visita_ss']) && $data['visita_ss'] === 'No') continue;
                if ($field === 'bpm2' && isset($data['visita_dagma']) && $data['visita_dagma'] === 'No') continue;
                if ($field === 'bpm3' && isset($data['visita_west']) && $data['visita_west'] === 'No') continue;
                if ($field === 'bpm8' && isset($data['novedad_grameras']) && $data['novedad_grameras'] === 'No') continue;
                
                // CAMPOS TI
                if ($field === 'ti' && isset($data['equipos_ti']) && $data['equipos_ti'] === 'No') continue;
                if ($field === 'ti1' && isset($data['facturas_ti']) && $data['facturas_ti'] === 'No') continue;
                if ($field === 'ti2' && isset($data['novedades_ti']) && $data['novedades_ti'] === 'No') continue;
                if ($field === 'ti3' && isset($data['casos_ti']) && $data['casos_ti'] === 'No') continue;
                
                // CAMPOS SST
                if ($field === 'sst1' && isset($data['accidentes_sst']) && $data['accidentes_sst'] === 'No') continue;
                if ($field === 'sst2' && isset($data['incapacidades_sst']) && $data['incapacidades_sst'] === 'No') continue;
                if ($field === 'sst3' && isset($data['ambiente_laboral']) && $data['ambiente_laboral'] === 'No') continue;
                if ($field === 'sst4' && isset($data['senal_sst']) && $data['senal_sst'] === 'No') continue;
                if ($field === 'sst6' && isset($data['entrega_epp']) && $data['entrega_epp'] === 'No') continue;
                if ($field === 'sst7' && isset($data['novedades_sst']) && $data['novedades_sst'] === 'No') continue;
                if ($field === 'sst8' && isset($data['casos_sst']) && $data['casos_sst'] === 'No') continue;
                
                // CAMPOS MANTENIMIENTO
                if ($field === 'mant' && isset($data['equipos_cocina']) && $data['equipos_cocina'] === 'No') continue;
                if ($field === 'mant1' && isset($data['equipos_bar']) && $data['equipos_bar'] === 'No') continue;
                if ($field === 'mant2' && isset($data['equipos_salon']) && $data['equipos_salon'] === 'No') continue;
                if ($field === 'mant3' && isset($data['locativos']) && $data['locativos'] === 'No') continue;
                if ($field === 'mant4' && isset($data['pendientes']) && $data['pendientes'] === 'No') continue;
                if (in_array($field, ['mant5', 'mant6', 'mant7', 'mant8'], true) && isset($data['planta_elect']) && $data['planta_elect'] === 'No') continue;
                
                // ÁREA DE BAR
                if ($field === 'hielo' && isset($data['hielo_produ']) && $data['hielo_produ'] === 'No') continue;
                if ($field === 'hielo1' && isset($data['hielo_kolbitos']) && $data['hielo_kolbitos'] === 'No') continue;
                if ($field === 'hielo2' && isset($data['hielo_consumo'])  && $data['hielo_consumo']  === 'No') continue;
                if ($field === 'hielo4' && isset($data['hielo_enviado']) && $data['hielo_enviado'] === 'No') continue;
                if ($field === 'hielo5' && isset($data['hielo_recibido']) && $data['hielo_recibido'] === 'No') continue;
                
                // CAMPOS TESORERIA
                if ($field === 'fa_mesas' && isset($data['facturas_mesas']) && $data['facturas_mesas'] === 'No') continue;
                if ($field === 'fa_dom' && isset($data['facturas_domic']) && $data['facturas_domic'] === 'No') continue;
                if ($field === 'fa_rappi' && isset($data['facturas_rappi'])  && $data['facturas_rappi']  === 'No') continue;
                if ($field === 'tesor1' && isset($data['bonos_coomeva']) && $data['bonos_coomeva'] === 'No') continue;
                if ($field === 'tesor2' && isset($data['easypedido']) && $data['easypedido'] === 'No') continue;

                // CAMPO MERCADEO
                if ($field === 'mer4' && isset($data['reservas_15']) && $data['reservas_15'] === 'No') continue;
    ...
    if (!$_POST || hasEmptyFields($_POST, $fields)){
        ...
        switch($data['sede']){
            case 'CHIPICHAPE':
                ...
                $mail->addAddress('chipichape@misterwings.com');
                    break;
                    
            case 'FLORA':
                $mail->addAddress('coordinadorflora@misterwings.com');
                $mail->addAddress('laflora@misterwings.com');
                    break;
        }
        
        $email_list = [
            'adminlaflora@misterwings.com','coordinador.sistemas@misterwings.com','soporte@misterwings.com',
            ...
            'mejoramiento@misterwings.com','mercadeo@misterwings.com','lenisalvaro@hotmail.com','auxiliar.sg-sst@misterwings.com',
            'gerencia@misterwings.com','mantenimiento@misterwings.com', 'coord.inventarios@misterwings.com','visual@misterwings.com',
            'contabilidad.valquin.les@misterwings.com', 'comercial@misterwings.com','esquin@hotmail.com',
        ...
        // Helper para escape seguro + saltos de línea
        function e($s) {
        ...
        function sanitize_number($val, $is_int = false) {
            if ($val === null) return $is_int ? 0 : 0.0;
            $s = trim((string)$val);
            if ($s === '') return $is_int ? 0 : 0.0;
            $s = str_replace(['$', '%', ' '], '', $s);
